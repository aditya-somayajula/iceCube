...
# ---------- ---------- ---------- Main Function ---------- ---------- ---------- #
def main():
    """Display Data Reference Page elements"""
    with st.sidebar:
        st.caption("_Perform data reference activities in the IceCube application_")
        st.button("Define", type="primary")
        st.button("Curate", type="primary")
# ---------- ---------- ---------- ---------- ---------- ---------- ---------- #
...
if __name__ == "__main__":
    ...
    if st.session_state["authenticated"]:
        st.session_state["page"] = "Data Reference"
    else:
    ...
    with col2:
        st.markdown("""<style>.centered-title {text-align: center;}</style><h1 class="centered-title">Data Reference</h1>""", unsafe_allow_html=True)
    with col3:
