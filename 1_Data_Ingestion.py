...
# ---------- ---------- ---------- Main Function ---------- ---------- ---------- #
def main():
    """Display Data Ingestion Page elements"""
    with st.sidebar:
        st.caption("_Perform data ingestion activities in the IceCube application_")
        st.button("Load", type="primary")
        st.button("Transform", type="primary")
        st.button("Configure", type="primary")
# ---------- ---------- ---------- ---------- ---------- ---------- ---------- #
...
if __name__ == "__main__":
    ...
    with col2:
        st.markdown("""<style>.centered-title {text-align: center;}</style><h1 class="centered-title">Data Ingestion</h1>""", unsafe_allow_html=True)
    with col3:
